...
const ProductRating = ({ ratings }) => {
    ...
    const renderStarRating = (averageRating) => {
        ...
        for (let i = 0; i < fullStars; i++) {
            stars.push(<span key={`full-star-${i}`}><img style={{ width: '20px', height: '20px' }} src={starImage} alt="" /></span>);
        }

        for (let i = 0; i < halfStars; i++) {
            stars.push(<span key={`half-star-${i}`}><img style={{ width: '20px', height: '20px' }} src={halfStarImage} alt="" /></span>);
        }

        for (let i = 0; i < emptyStars; i++) {
            stars.push(<span key={`empty-star-${i}`}><img style={{ width: '20px', height: '20px' }} src={emptyStarImage} alt="" /></span>);
        }
    ...
    return (
        <div>
            <div ><Button onClick={() => setShowModal(true)}>Calificar</Button>  {averageRating}</div>
                    ...
                    <div>
                        ...
                        <div>
                            {[...Array(5)].map((_, index) => (
                                ...
                                >
                                    <img style={{ cursor: 'pointer', width: '40px', height: '40px' }} src={starImage} alt="" />
                                </span>
                            ))}
                        </div>
                    </div>
                    <DialogActions>
                        <Button onClick={() => handleSubmitRating(selectedRating, id)}>Enviar puntuación</Button>
                    </DialogActions>
                </Dialog>
            )}
        </div>
